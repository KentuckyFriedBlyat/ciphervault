...
class SdrNoiseSource:
    ...
    def resolve_frequency(self, base_freq, auto_sweep=True, log=print):
        ...
        Probes `base_freq` first (a short RAM-only sample). If the front end is
        DEAD or OVERLOADED there - or the spectrum has too little spread - it
        sweeps upward from the calibrated lower limit all the way to the upper
        limit until a clean spot is found. Consecutive dead probes abort early:
        no frequency rotation fixes an antenna/coax problem.
        ...
        # Use calibrated limits if available, otherwise fall back to defaults
        if LOWER_LIMIT_FREQ_HZ is None:
            LOWER_LIMIT_FREQ_HZ = int(SWEEP_START_MHZ * 1e6)
        if UPPER_LIMIT_FREQ_HZ is None:
            UPPER_LIMIT_FREQ_HZ = int(SWEEP_MAX_MHZ * 1e6)

        log("  [WARN] %s - sweeping from %.1f MHz to %.1f MHz in %d kHz steps..."
            % (reason, LOWER_LIMIT_FREQ_HZ / 1e6, UPPER_LIMIT_FREQ_HZ / 1e6, SWEEP_STEP_KHZ))
        ...
        if saw_overload:
            raise CaptureError("no usable frequency in %.0f-%.0f MHz: overload persists across the "
                               "whole sweep - add a 10-20 dB attenuator or lower GAIN and retry"
                               % (lo / 1e6, hi / 1e6))
        raise CaptureError("no usable frequency in %.0f-%.0f MHz (%d probes) - every spot was "
                           "dead or too structured" % (lo / 1e6, hi / 1e6, probes))
